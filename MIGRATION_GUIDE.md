# Migration Guide: Sonline 0.x to Sonline 2.0+

This guide will help you migrate from Sonline 0.x (legacy) to Sonline 2.0+, the new user-aware agent framework.
...
Sonline 2.0+ represents a fundamental architectural shift from a simple LLM wrapper to a full-fledged **user-aware agent framework**. Here are the major changes:
...
- ❌ Monolithic `SonlineBase` class → Modular tool system
...
# Run server
server = SonlineFastAPIServer(agent)
...
**Best for:** Teams that want to adopt Sonline 2.0+ gradually while maintaining existing code.

#### Step 1: Install Sonline 2.0+

```bash
pip install 'Sonline[flask,anthropic]'
```

#### Step 2: Wrap Your Existing SonlineBase Instance
...
# vn = MySonline(config={'model': 'gpt-4', 'api_key': 'your-key'})
# vn.connect_to_postgres(...)
# vn.train(ddl="...")
# etc.
...
class SimpleUserResolver(UserResolver):
    async def resolve_user(self, request_context: RequestContext) -> User:
        user_email = request_context.get_cookie('Sonline_email')
        if not user_email:
            raise ValueError("Missing 'Sonline_email' cookie")
...
# This automatically registers run_sql and memory tools from your SonlineBase instance
tools = LegacySonlineAdapter(vn)
...
agent = Agent(
    llm_service=llm,
    tool_registry=tools,  # LegacySonlineAdapter is a ToolRegistry
...
**What the LegacySonlineAdapter does:**
...
**Cons:**
- ⚠️ Limited user awareness (all requests use same SonlineBase instance)
...
```python
from Sonline import SonlineBase
from Sonline.openai_chat import OpenAI_Chat
from Sonline.chromadb import ChromaDB_VectorStore
...
from Sonline.integrations.postgres import PostgresRunner
...
| Feature | Sonline 0.x | Sonline 2.0+ |
|---------|-----------|------------|
